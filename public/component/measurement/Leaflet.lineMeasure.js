...
(function (factory) {
    ...
    L.Control.lineMeasure = L.Control.extend({
        ...
        options: {
             ...
             * @default
             */
            measureControlTitleOn: "Turn On Measure Distances",
            /**
             * Title for the control going to be switched off
             * @type {String}
             * @default
             */
            measureControlTitleOff: "Turn off Measure Distances",
            /**
             ...
             * @default
             */
            clearControlTitle: 'Clear Measure Distances',
            /**
             ...
             * @default
             */
            unitControlUnits: ["kilometres" , "metres", "nauticalmiles"],
            /**
        ...
        _getDistance: function (distance) {
            ...
            } else if(this.options.unit === 'metres') {
        ...
        _drawArrow: function (arcLine) {
            ...
            if (!this._currentLine){  // just bind tooltip if not drawing line anymore, cause following the instruction of tooltip is just possible when not drawing a line
                newArrowMarker.bindPopup (this.options.tooltipTextAdd, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
            }
        ...
        _startLine: function (clickCoords) {
            ...
            this._currentLine = {
                ...
                handleMarkers: function (latlng) {
                    ...
                    if (lastCircleMarker) {
                        lastCircleMarker.bindPopup (polylineState.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                    ...
                    newCircleMarker.bindPopup (polylineState.options.tooltipTextFinish + polylineState.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                ...
                finalize: function() {
                    ...
                    if (this.circleCoords.length > 1) {
                        ...
                        lastCircleMarker.unbindPopup ();  // to close the opened Tooltip after it's been opened after click onto point to finish the line
                        polylineState._currentLine.circleMarkers.map (function (circle) {circle.bindPopup (polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'})});
                        polylineState._currentLine.circleMarkers[0].bindPopup (polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete + polylineState.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                        lastCircleMarker.bindPopup (polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete + polylineState.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                        polylineState._currentLine.arrowMarkers.map (function (arrow) {arrow.bindPopup (polylineState.options.tooltipTextAdd, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'})});
                        lastCircleMarker.off ('click', polylineState._finishPolylinePath, polylineState);
        ...
        _resumePolylinePath: function (e) {
            if (e.originalEvent.ctrlKey === true) {    // just resume if user pressed the CTRL-Key while clicking onto the last circle
                ...
                this._currentLine.circleMarkers.last().unbindPopup();   // remove popup-tooltip of previous, last circleMarker
                this._currentLine.circleMarkers.last().bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
        ...
        _clickedArrow: function(e) {
            if (e.originalEvent.ctrlKey) {
                ...
                newCircleMarker.bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
        ...
        _dragCircleMouseup: function () {
            ...
            if ((this._circleNr === 0) || (this._circleNr === this._arrPolylines[this._lineNr].circleCoords.length-1)) {
               this._e1.target.bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
            } else {
               this._e1.target.bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
            }
            this._resetPathVariables();
            this._map.off ('mousemove', this._dragCircleMousemove, this);
        ...
        _dragCircleMousemove: function (e2) {
            ...
            this._e1.target.unbindPopup();    // unbind popup-tooltip cause otherwise it would be annoying during dragging, or popup instantly again if it's just closed
        ...
        _resumeFirstpointClick: function (e) {
            ...
            this._arrPolylines[lineNr].circleMarkers [0].unbindPopup();
            this._arrPolylines[lineNr].circleMarkers [0].bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
            ...
            newCircleMarker.on ('mousedown', this._dragCircle, this);
            newCircleMarker.bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
        ...
        _dragCircle: function (e1) {
            ...
            if (e1.originalEvent.ctrlKey) {   // if user wants to resume drawing a line
                ...
                if (e1.target.cntCircle === 0) {
                    ...
                    text = text + '<div class="polyline-measure-tooltip-difference">+' + '0</div>';//difference is here
                    text = text + '<div class="polyline-measure-tooltip-total">' + '0</div>';
            ...
            if (e1.originalEvent.shiftKey) {    // it's not possible to use "ALT-Key" instead, cause this won't work in some Linux distributions (there it's the default hotkey for moving windows)
                ...
                if (this._arrPolylines[lineNr]) {
                    ...
                    if (circleNr === 0) {
                        ...
                        this._arrPolylines[lineNr].circleMarkers [0].bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                        this._arrPolylines[lineNr].arrowMarkers [circleNr].removeFrom (this._layerPaint);
                        ...
                        this._arrPolylines[lineNr].circleMarkers [circleNr-1].bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                ...
                // if this is the first line and it's not finished yet
                  ...
                  if(this._currentLine.circleMarkers.length === 1) {
                    ...
                    if(this._currentLine.circleMarkers.length === 1) {
                        ...
                        lineCoords.splice (0, arcpoints-1);
                        this._currentLine.circleMarkers [0].bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
                        ...
                        this._currentLine.circleMarkers [circleNr-1].bindPopup (this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {direction:'top', opacity:0.7, className:'polyline-measure-popupTooltip'});
...
//======================================================================================
    ...
    L.Map.addInitHook(function () {
        if (this.options.polylineMeasureControl) {
            this.PMControl = new L.Control.lineMeasure();
